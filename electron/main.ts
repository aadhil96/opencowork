...
// pptxgenjs is CJS — same treatment. Constructor is the module export.
// eslint-disable-next-line @typescript-eslint/no-explicit-any
...
// Export an assistant analysis/memo to a file the user picks (DOCX / PPTX / Markdown / TXT).
ipcMain.handle('export-document', async (_event, args: { defaultName: string; format: 'docx' | 'pptx' | 'md' | 'txt'; content: string }) => {
  ...
  const result = await dialog.showSaveDialog({
    ...
    filters: [
      ...
      : format === 'pptx' ? { name: 'PowerPoint Presentation', extensions: ['pptx'] }
  ...
  try {
    if (format === 'docx') {
      ...
      const pptx = markdownToPptx(content, defaultName)
      const buf = await pptx.write({ outputType: 'nodebuffer' })
      writeFileSync(result.filePath, buf as Buffer)
    } else {
...
// Minimal markdown → pptx: each top-level heading (# / ##) starts a new slide;
// bullets/paragraphs become the slide body; inline marks are stripped.
function markdownToPptx(md: string, title: string): any {
  const strip = (s: string) =>
    s.replace(/\[([^\]]+)\]\([^)]+\)/g, '$1').replace(/\*\*|\*|`|__|~~/g, '').trim()

  type Bullet = { text: string; level: number }
  type Slide = { title: string; bullets: Bullet[] }
  const slides: Slide[] = []
  ...
  for (const raw of md.split(/\r?\n/)) {
    ...
    if (heading) {
      ...
      const text = strip(heading[2])
      if (level <= 2) { flush(); cur = { title: text, bullets: [] } }
      else { if (!cur) cur = { title, bullets: [] }; cur.bullets.push({ text, level: 0 }) }
      continue
    }
    ...
    if (bullet) {
      ...
      cur.bullets.push({ text: strip(bullet[2]), level: Math.min(4, Math.floor(bullet[1].length / 2)) })
      continue
    }

    // Skip table rows / horizontal rules; treat other text as a body line.
    if (/^\|/.test(line) || /^[-=_]{3,}$/.test(line)) continue
    if (!cur) cur = { title, bullets: [] }
    cur.bullets.push({ text: strip(line), level: 0 })
  }
  flush()
  if (slides.length === 0) slides.push({ title, bullets: [] })

  const pptx = new PptxGenJS()
  pptx.layout = 'LAYOUT_WIDE'
  pptx.defineSlideMaster({ title: 'OCW', background: { color: 'FFFFFF' } })

  // Title slide
  const title0 = pptx.addSlide()
  title0.addText(title || slides[0].title, { x: 0.5, y: '42%', w: '90%', h: 1.2, fontSize: 32, bold: true, align: 'center', color: '1A1A1A' })
  title0.addText('Generated by OpenCowork', { x: 0.5, y: '60%', w: '90%', h: 0.4, fontSize: 12, align: 'center', color: '888888' })

  for (const s of slides) {
    const slide = pptx.addSlide()
    slide.addText(s.title || 'Slide', { x: 0.5, y: 0.35, w: '90%', h: 0.8, fontSize: 24, bold: true, color: '1A1A1A' })
    if (s.bullets.length > 0) {
      const body = s.bullets.map(b => ({
        text: b.text,
        options: { bullet: { indent: 16 }, indentLevel: b.level, fontSize: 16, color: '333333', breakLine: true, paraSpaceAfter: 6 }
      }))
      slide.addText(body as any, { x: 0.6, y: 1.4, w: '88%', h: 5.4, valign: 'top' })
    }
  }
  return pptx
}
